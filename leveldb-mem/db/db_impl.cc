...
DBImpl::DBImpl(const Options& raw_options, const std::string& dbname)
    : env_(raw_options.env),
      ...
      m_versions_(new MemVersionSet(dbname_, &options_,
                               &internal_comparator_,versions_)){}
...
void DBImpl::RecordMemTable(MemTable* mem, MemVersionEdit* edit,
    ...
    if (meta.table_size > 0) {
      edit->AddTable(0, meta.number, meta.table_size,meta.mem_table,meta.smallest,
                    meta.largest);
    }
...
void DBImpl::CompactMemTable() {
  ...
  MemVersionEdit edit;
  MemVersion* base = m_versions_->current();
  base->Ref();
  RecordMemTable(imm_,&edit, base);//add meta data
  base->Unref();
  Status s;
...
void DBImpl::MaybeScheduleCompaction() {
  ...
  } else if (imm_ == nullptr && !m_versions_->NeedsCompaction() && manual_compaction_ == nullptr &&
...
void DBImpl::OpenCompactionOutputTable(CompactionState* compact){
  ...
  uint64_t table_number;
  {
    ...
    all_memtables_.insert(std::make_pair(table_number,out.mem_table));//新生成的memtable加入
...
void DBImpl::InstallMemCompactionResults(CompactionState* compact) {
  ...
  for (size_t i = 0; i < compact->outputs.size(); i++) {
    const CompactionState::Output& out = compact->outputs[i];
    compact->m_compaction->edit()->AddTable(1, out.number, out.table_size, out.mem_table,
...
Status DBImpl::DoMemCompactionWork(CompactionState* compact) {
  ...
  while (input->Valid() && !shutting_down_.load(std::memory_order_acquire)) {
    ...
    input->Next();//没有调用mergeIte的next,而是memtable的迭代器？
  }
  Status status;
  if (shutting_down_.load(std::memory_order_acquire)) {
...
Status DBImpl::Get(const ReadOptions& options, const Slice& key,
  ...
  // Unlock while reading from files and memtables
  {
    ...
    if (mem->Get(lkey, value, &s)) {
      ...
      //___________
      if(s.ok())std::fprintf(stdout,"get from versions\n");
      if(!s.ok()){
  ...
  if (have_stat_update && current->UpdateStats(stats)) {
    MaybeScheduleCompaction();//maybe seek compact
  }
  mem->Unref();
  ...
  while (true) {
    if (!bg_error_.ok()) {
      ...
      mutex_.Lock();
               ...
               (mem_->GetSize() <= options_.max_table_size)) {
                 //单个mem还有空间
      ...
      background_work_finished_signal_.Wait();
    }else if(m_versions_->NumLevelTables(0) >=config::kMemL1_StopWritesTrigger){
      //m1层表太多，等待合并完成，这里由于m2满了，应该一直等着
      Log(options_.info_log, "Too many M1 tables; waiting...\n");
      //_______
      std::fprintf(stdout, "Too many M1 tables; waiting...,numbers:%d\n",m_versions_->current()->NumTables(0));
      background_work_finished_signal_.Wait();
      //else if flush ToDo
